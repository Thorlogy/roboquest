    ...
    _createCodeLine(block, index) {
        ...
        line.innerHTML = `
            ...
            <div class="block-move-btns">
                <button class="block-move-btn btn-up" aria-label="Nach oben" ${index === 0 ? 'disabled' : ''}>▲</button>
                <button class="block-move-btn btn-down" aria-label="Nach unten" ${index === this.program.length - 1 ? 'disabled' : ''}>▼</button>
            </div>
            <button class="block-delete-btn" aria-label="Löschen">×</button>
        `;
