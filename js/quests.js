...
const ITEM_DEFS = {
    ...
    scrap: { color: 0x94a3b8, emissive: 0x475569, emoji: "⚙️", name: "Schrottteil", points: 50 }
};
...
function tryDropItem() {
    ...
    if (storyState.carriedItem.type === 'seed') {
        for (const spot of plantSpots) {
            if (!spot.planted && Math.hypot(rx - spot.x, rz - spot.z) < 2.5) {
                ...
                if (window.audioEngine) window.audioEngine.playHappyBeep();
                if (typeof window.addVitality === 'function') window.addVitality(15); 
    ...
    if (baseZone && Math.hypot(rx - baseZone.x, rz - baseZone.z) < baseZone.radius) {
        ...
        if (item.type === 'trash' && typeof window.addVitality === 'function') window.addVitality(5);
        checkQuestProgress();
    } else {
...
function checkQuestProgress() {
    // Standard-Quest-System im Missionsmodus unterdrücken
