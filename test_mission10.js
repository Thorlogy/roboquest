...
function checkCollision(x, z, yaw) {
...
// Imagine robot drives straight forward (z decreases) but at an angle
// To hit x=2.5, z=-5, robot starts at 0,0. Angle to target is ~26.5 degrees.
const yaw = Math.PI - 0.4636; // facing the box
// Ray intersection math is hard, just sample points along the line
for (let t=0; t<=10; t+=0.1) {
    const fDX = Math.sin(yaw), fDZ = Math.cos(yaw);
    const rx = 0 + fDX * t;
    const rz = 0 + fDZ * t;
    const collides = checkCollision(rx, rz, yaw);
