...
// ════════════════════════════════════════════════════════════════
/**
 * Baut das 3D-Modell des "Eco-Bots" aus Three.js-Primitiven zusammen.
 * Das fertige Modell wird in die globale Variable `roverGroup` geschrieben
 * und zur Szene hinzugefügt.
 */
function buildEcoBot() {
    roverGroup = new THREE.Group();
    window.roverGroup = roverGroup; // <-- WICHTIG: Überschreibe die globale Referenz!
    scene.add(roverGroup);

    // Farb-Palette des Roboters
    const mRustyYellow = new THREE.MeshLambertMaterial({ color: 0xd97706 }); // Dunkelgelb/Orange (Karosserie)
    const mRustyOrange = new THREE.MeshLambertMaterial({ color: 0xeab308 }); // Hellgelb (Akzente)
    const mDark = new THREE.MeshLambertMaterial({ color: 0x1e293b });        // Dunkelblau/Grau (Reifen/Mechanik)
    const mSilver = new THREE.MeshLambertMaterial({ color: 0x94a3b8 });      // Silber/Grau (Metallteile)
    const mCyan = new THREE.MeshBasicMaterial({ color: 0x22d3ee });          // Leuchtendes Cyan (Lichter/Augen)
    const mSolar = new THREE.MeshLambertMaterial({ color: 0x1e3a8a });       // Dunkelblau (Solarpanels)
